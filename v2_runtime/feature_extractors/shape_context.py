...
def get_shape_context(unorganized_pc):
    ...
    features = torch.tensor(compute_3d_shape_context(unorganized_pc), dtype=torch.float32, device=device)
    print(features.shape)

    # fps the centers out
    # unorganized_pc_no_zeros expected (1,n,3)
    unorganized_pc_no_zeros = torch.tensor(unorganized_pc, dtype=torch.float32, device=device).unsqueeze(dim=0)
    ...
    return agg_point_feature, torch.tensor(unorganized_pc), unorganized_pc_no_zeros, center
